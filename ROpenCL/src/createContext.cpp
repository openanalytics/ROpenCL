...
SEXP getKernelWorkGroupInfo(SEXP sKernel, SEXP sDeviceID){
    ...
    if (!logError( clGetKernelWorkGroupInfo(*kernel, *deviceID, CL_KERNEL_COMPILE_WORK_GROUP_SIZE, sizeof(sizes), &sizes, NULL) )) {
        Rcpp::List wgs;
...
#endif
#ifdef CL_KERNEL_GLOBAL_WORK_SIZE /* 3x size_t */
    sizes[0] = sizes[1] = sizes[2] = 0;
    if (!logError( clGetKernelWorkGroupInfo(*kernel, *deviceID, CL_KERNEL_GLOBAL_WORK_SIZE, sizeof(sizes), &sizes, NULL) )) {
        Rcpp::List gws;
...
void enqueueWriteBufferFloatVector(SEXP sQueue, SEXP sMemBuffer, SEXP sGlobalWorkSize, SEXP sObject){
    ...
    printf("ROpenCL::enqueueWriteBufferFloatVector()\n");
...
void enqueueWriteBufferIntegerVector(SEXP sQueue, SEXP sMemBuffer, SEXP sGlobalWorkSize, SEXP sObject){
    ...
    printf("ROpenCL::enqueueWriteBufferIntegerVector()\n");
...
void enqueue1DRangeKernel(SEXP sQueue, SEXP sKernel, SEXP sGlobalWorkSize, SEXP sLocalWorkSize){
    ...
    std::cout << "localWorkSize=" << localWorkSizePtr << ":" << localWorkSize << "\n";
